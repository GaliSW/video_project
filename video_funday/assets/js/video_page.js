let app = new Vue({
    ...
    methods: {
                        ...
                        switch (e.data) {
                            case 0: //=== 結束 ===
                                ...
                                if (app.mode == 1) {
                                    ...
                                    location.href = `./video_funday.html?id=${app.next}&mid=${app.mid}&cid=${app.cid}`;
                                } else if (app.mode == 2) {
                                    location.href = `./video_funday.html?id=${app.random}&mid=${app.mid}&cid=${app.cid}`;
                                } else {
                                    player.stopVideo();
                                }
                                break;
        ...
        audioList() {
            axios
                .get(
                    `https://funday.asia/api/ProgramWeb/RecordingListNew.asp?indx=${this.videoId}&member_id=${this.mid}`
                )
                .then((response) => {
                    // console.log(response);
        ...
        vdPrev() {
            if (this.mode == 0) {
                location.href = `./video_funday.html?id=${this.prev}&mid=${app.mid}&cid=${app.cid}`;
            } else if (this.mode == 2) {
                location.href = `./video_funday.html?id=${this.random}&mid=${app.mid}&cid=${app.cid}`;
            } else {
                player.seekTo(0);
                player.playVideo();
            }
        ...
        vdNext() {
            if (this.mode == 0) {
                location.href = `./video_funday.html?id=${this.next}&mid=${app.mid}&cid=${app.cid}`;
            } else if (this.mode == 2) {
                location.href = `./video_funday.html?id=${this.random}&mid=${app.mid}&cid=${app.cid}`;
            } else {
                player.seekTo(0);
        ...
        share(status) {
            let singer;
            if (status == 1) {
                const originalUrl = `https://tube.funday.asia/video.html?id=${this.videoId}`;
                this.URL = `${originalUrl}#mid=${app.singerId}`;
                singer = `mid=${this.singerId}`;
                //分享配音時的cid - mid (411-213132)
            } else if (status == 0) {
                this.URL = `https://tube.funday.asia/video.html?id=${this.videoId}`;
                singer = "";
